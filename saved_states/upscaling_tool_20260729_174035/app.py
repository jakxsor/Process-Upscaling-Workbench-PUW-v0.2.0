  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 980px) {
      .flow-field-grid { grid-template-columns: 1fr; }
    }
...
<body>
  ...
  <main id="appMain">
    ...
    <section class="panel">
      ...
      <div class="panel-body">
        ...
        <div id="stepFlowInspector" class="step-flow-inspector empty">Select a block to add inputs, outputs, and waste streams.</div>
  ...
  <script>
    ...
    function selectedBlock() {
      ...
      if (block) ensureBlockFlowFields(block);
      return block;
    }

    function ensureBlockFlowFields(block) {
      if (!Array.isArray(block.inputs)) block.inputs = [];
      if (!Array.isArray(block.outputs)) block.outputs = [];
      if (!Array.isArray(block.wastes)) block.wastes = [];
      return block;
    }
    ...
    function blockCardHtml(block) {
      ...
      const flowCounts = [
        block.inputs.length ? `I:${block.inputs.length}` : "",
        block.outputs.length ? `O:${block.outputs.length}` : "",
        block.wastes.length ? `W:${block.wastes.length}` : ""
      ].filter(Boolean).join(" ");
      return `
    ...
    function renderStepFlowInspector() {
      ...
      if (!block) {
        ...
        root.innerHTML = "Select a block to add inputs, outputs, and waste streams.";
        return;
      }
      ...
      root.innerHTML = `
        <div class="step-flow-head">
          <div>
            <div class="label">Step Material Flow</div>
            <strong>${escapeHtml(block.id)}</strong>
            <span class="pill">${escapeHtml(block.behavior)}</span>
          </div>
          <span class="muted small">One item per line, or separate with commas/semicolons.</span>
        </div>
        <div class="flow-field-grid">
          <label>
            <div class="label">Inputs</div>
            <textarea data-flow-field="inputs" placeholder="benzophenone&#10;cyclohexane&#10;NH4OAc">${escapeHtml(block.inputs.join("\n"))}</textarea>
          </label>
          <label>
            <div class="label">Outputs</div>
            <textarea data-flow-field="outputs" placeholder="reaction mixture&#10;organic phase&#10;purified product">${escapeHtml(block.outputs.join("\n"))}</textarea>
          </label>
          <label>
            <div class="label">Waste / Emissions</div>
            <textarea data-flow-field="wastes" placeholder="aqueous waste&#10;brine waste&#10;VOC vent">${escapeHtml(block.wastes.join("\n"))}</textarea>
          </label>
        </div>
      `;
      root.querySelectorAll("[data-flow-field]").forEach(field => {
        field.addEventListener("input", event => {
          ...
          if (!current) return;
          current[event.target.dataset.flowField] = parseFlowList(event.target.value);
          renderExport();
        });
      });
    }
    ...
    function renderExport() {
      ...
      $("jsonOut").textContent = JSON.stringify({
        workflow: "source text -> annotated blocks -> material inputs/outputs/waste -> behavior presets -> phenomenon groups -> task/unit alternatives",
        text: state.text,
        blocks,
        materialFlow,
        groups,
